...
class AutoGLMApplication : Application() {
    ...
    companion object {
        ...
        fun getInstance(): AutoGLMApplication? = instance
    }

    // 广播接收器，用于接收停止任务信号
    private val stopTaskReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.action == "com.example.autoglm.ACTION_STOP_TASK") {
    ...
    override fun onCreate() {
        ...
        val filter = IntentFilter("com.example.autoglm.ACTION_STOP_TASK")
